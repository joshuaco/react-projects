...
function ContactInfo({ client }: ContactInfoProps) {
  return (
      ...
      <div className="p-4">
        <div className="flex flex-col items-center pb-6 border-b">
          <div className="h-20 w-20 rounded-full bg-blue-500 flex items-center justify-center text-white text-xl mb-3">
            {client.name.split(' ').map(name => name.charAt(0)).join('')}
          </div>
          <h3 className="font-semibold text-lg">{client.name}</h3>
          <p className="text-sm text-muted-foreground">{client.currentPlan} Account</p>
